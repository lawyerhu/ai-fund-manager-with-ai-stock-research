...
EVIDENCE_PACKET_INSTRUCTIONS = (
    ...
    "earnings/revenue/FCF revisions, valuation expansion or multiple expansion versus operating improvement, catalyst pricing and downside "
    ...
    "outside this research layer and must not be invented. Apply Alpha / Mispricing Discipline explicitly: "
    "distinguish Business Quality, Fundamental Direction, Market Expectations, Remaining Mispricing and Remaining Alpha. "
    "Business Quality is not Alpha. Good news is not Alpha. Mispricing is the potential source of Alpha. "
    "For each final Top 5 item, record alpha_thesis, market_expectation and remaining_alpha_view. "
    "alpha_thesis must state what the market may be wrong about, why the gap may remain and what would invalidate it; "
    "use UNKNOWN when evidence is insufficient. market_expectation must be UNKNOWN, or begin with OBSERVED: / INFERRED: "
    "and explain its basis; INFERRED is not a verified fact, and no implied EPS, growth rate or valuation may be invented. "
    "remaining_alpha_view is a qualitative current-price judgment, not a score or automatic action. "
    "Event itself is not Alpha. Treat event evidence only as Reveal, Validate, Accelerate or Invalidate; "
    "neither a near-term event nor a catalyst is required, and event presence must not automatically boost rank."
)
...
def main(argv=None):
    ...
    for index, symbol in enumerate(top5, start=1):
        prompt = (
            ...
            "weakens or leaves unresolved. Also record alpha_thesis, market_expectation and remaining_alpha_view "
            "for this stock using UNKNOWN or an OBSERVED:/INFERRED: explanation; these are qualitative research "
            "fields, not scores or automatic actions. "
    ...
    final_prompt = (
        ...
        "fixed factor weights or vetoes. Each final ranking item must explicitly include alpha_thesis, "
        "market_expectation and remaining_alpha_view. Use UNKNOWN rather than fabricating a field; for "
        "market_expectation use exactly UNKNOWN or an OBSERVED:/INFERRED: explanation with its evidence basis. "
    ...
    if previous_supplemental is None:
        previous_prompt = (
            ...
            "Record alpha_thesis, market_expectation and remaining_alpha_view using UNKNOWN or an "
            "OBSERVED:/INFERRED: explanation with its basis; these are qualitative research fields, not scores or "
            "automatic actions. Return compact structured DeepDiveResearch JSON only.\n"
    ...
    comparison_prompt = (
        ...
        "style, quality or incumbent prior. Compare Incumbent Remaining Alpha directly with Challenger Remaining Alpha. "
        "Assume that today neither security is held and the same capital is available at current prices; decide which "
        "has the more credible and larger remaining expectation gap after reasonable research-layer friction. "
